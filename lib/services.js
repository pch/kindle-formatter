...
const extractPageNumber = (text) => {
  return text.match(/Page\s+([0-9]+)/)[1];
};

const extractLocation = (text) => {
  return text.match(/Location\s+([0-9]+)/)[1];
};
...
export const parseHighlights = (rawHTML) => {
  ...
  const bookTitle = doc.querySelector("h1 .bookTitle");
  const bookAuthors = doc.querySelector("h1 .authors");
...
export const bookToMarkdown = (book) => {
  ...
  book.sections.forEach((section) => {
    ...
    section.highlights.forEach((hl) => {
      output += `\t\t- ${hl.content}\n`;
      output += `\t\t\t- **${hl.type}** (page: ${hl.page}, location: ${hl.location})\n`;
    });
  });
